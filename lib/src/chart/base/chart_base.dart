...
class _SfCartesianChartState extends State<SfCartesianChart>
  ...
  void createAndUpdateSeriesRenderer([SfCartesianChart oldWidget]) {
    if (widget.series != null && widget.series.isNotEmpty) {
      final List<CartesianSeriesRenderer> oldSeriesRenderers = seriesRenderers;
      seriesRenderers = <CartesianSeriesRenderer>[];
      ...
      for (int i = 0; i < seriesLength; i++) {
        ...
        if (oldWidget != null) {
          ...
          index = i < oldWidget._chartState.seriesRenderers.length &&
                  _isSameSeries(oldWidget.series[i], series)
              ? i
              : getExistingSeriesIndex(series, oldWidget);
        }
        // Create and update the series list here
        CartesianSeriesRenderer seriesRenderer;

        if (index != null &&
            index < oldSeriesRenderers.length &&
            oldSeriesRenderers[index] != null) {
          seriesRenderer = oldSeriesRenderers[index];
        } else {
  ...
  int getExistingSeriesIndex(CartesianSeries<dynamic, dynamic> currentSeries,
      SfCartesianChart chartWidget) {
    if (currentSeries.key != null) {
      for (int index = 0; index < chartWidget.series.length; index++) {
        final CartesianSeries<dynamic, dynamic> series =
            chartWidget.series[index];
  ...
  void _redraw() {
    ...
    if (widget._chartState._isLegendToggled) {
      ...
      widget._chartState._oldSeriesVisible =
          List<bool>(widget._chartSeries.visibleSeriesRenderers.length);
      for (int i = 0;
    ...
    widgetNeedUpdate = false;
    oldSeriesRenderers = widget._chartSeries.visibleSeriesRenderers;
    _getKeys(oldSeriesRenderers);
  ...
  void setPainterKey(int index, String name, bool renderComplete) {
    ...
    for (int i = 0; i < painterKeys.length; i++) {
      ...
      if (value >= painterKeys.length && !triggerLoaded) {
        ...
        _renderDuration = endTime.difference(_startTime);
        // Future<void>.delayed(const Duration(milliseconds: 0), () {
        //   //Chart fully loaded here.
        // });
        if (widget._chartState.renderDataLabel == null)
          widget._chartState.initialRender = false;
      }
    }
  }
  ...
  List<Widget> _bindCartesianLegendTemplateWidgets() {
    ...
    if (widget.legend.isVisible && widget.legend.legendItemBuilder != null) {
      for (int i = 0; i < widget._chartState.seriesRenderers.length; i++) {
        final CartesianSeriesRenderer seriesRenderer =
            widget._chartState.seriesRenderers[i];
  ...
  void _findVisibleSeries(BuildContext context) {
    ...
    for (int i = 0; i < seriesRenderers.length; i++) {
      seriesRenderers[i]._seriesName =
          seriesRenderers[i]._series.name ?? 'Series $i';
      final CartesianSeries<dynamic, dynamic> cartesianSeries =
          seriesRenderers[i]._series;
      final List<CartesianSeriesRenderer> oldSeriesRenderers =
          widget._chartState.oldSeriesRenderers;
      ...
      if (i == 0 ||
        ...
        if (!widget._chartState.initialRender &&
          ...
          if (i < visibleSeriesRenderers.length &&
            ...
            widget._chartState._oldSeriesVisible[i] =
                visibleSeriesRenderers[i]._visible;
          }
        }
        if (_legendCheck) {
        ...
        final CartesianSeriesRenderer cSeriesRenderer = widget._chartSeries
                    .visibleSeriesRenderers[visibleSeriesRenderers.length - 1]
...
class _ContainerArea extends StatelessWidget {
  ...
  void _calculateTrendlineRegion(
      SfCartesianChart chart, XyDataSeriesRenderer seriesRenderer) {
    if (seriesRenderer._series.trendlines != null) {
      for (Trendline trendline in seriesRenderer._series.trendlines) {
        if (trendline._isNeedRender)
          trendline.calculateTrendlinePoints(seriesRenderer, chart);
      }
    }
  }
    ...
    for (int i = 0; i < chart._chartSeries.visibleSeriesRenderers.length; i++) {
      ...
      if (_seriesRenderer != null && _seriesRenderer._visible) {
        ...
        if (selectionSettings != null) {
          ...
          selectionSettings._selectionRenderer.selectedSegments =
              chart._chartState.selectedSegments;
          selectionSettings._selectionRenderer.unselectedSegments =
        ...
        if (chart._chartState != null &&
            !chart._chartState._didSizeChange &&
            chart._chartState.animationController != null &&
            _series.animationDuration > 0 &&
            (chart._chartState._oldDeviceOrientation == null ||
                chart._chartState._oldDeviceOrientation ==
                    chart._chartState.deviceOrientation) &&
            (chart._chartState.zoomedState == null
                ? true
  ...
  CartesianSeriesRenderer _findSeries(Offset position) {
    ...
    for (int i = chart._chartSeries.visibleSeriesRenderers.length - 1;
      ...
      if (_seriesType == 'column' ||
        ...
        bool isSelect = false;
        seriesRenderer = chart._chartSeries.visibleSeriesRenderers[i];
        for (int k = chart._chartSeries.visibleSeriesRenderers.length - 1;
            k >= 0;
            k--) {
          isSelect = seriesRenderer._series.selectionSettings.enable &&
              ...
              ? seriesRenderer._series.selectionSettings._selectionRenderer
                  ._isSeriesContainsPoint(
                      chart._chartSeries.visibleSeriesRenderers[i], position)
              : false;
          if (isSelect) {
    ...
    if (chart._chartSeries.visibleSeriesRenderers != null &&
      ...
      if (!selectionSeriesRenderer._isOuterRegion &&
          selectionSeriesRenderer
                  ._series.selectionSettings._selectionRenderer !=
              null) {
        selectionSeriesRenderer._series.selectionSettings._selectionRenderer
            .seriesRenderer = selectionSeriesRenderer;
        selectionSeriesRenderer._series.selectionSettings
